...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        if role == "actor" and grit_cfg is not None and grit_cfg.get("enable", False):
            projectors_path = grit_cfg.get("projectors_path", None)
            if projectors_path is None:
                raise ValueError("grit.enable=True requires grit.projectors_path")
            projectors = load_projectors(projectors_path, map_location="cpu")
            attach_summary = attach_projectors_to_mlp_linears(
                actor_module,
                projectors,
                module_pattern=grit_cfg.get("module_pattern", "mlp"),
                strict=grit_cfg.get("strict_projector_attach", True),
            )
            if self.rank == 0:
                print(
                    "GRIT Phase 1 projectors attached: "
                    f"attached={len(attach_summary.attached)} "
                    f"missing={len(attach_summary.missing)} "
                    f"unexpected={len(attach_summary.unexpected)} "
                    f"shape_mismatch={len(attach_summary.shape_mismatch)}"
                )
        ...
        if (
            role == "actor"
            and grit_cfg is not None
            and grit_cfg.get("enable", False)
            and fsdp_strategy == "fsdp"
            and not fsdp_config.get("use_orig_params", False)
        ):
            raise ValueError(
                "GRIT Phase 1 gradient projection with FSDP requires "
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            if self.config.get("grit", None) is not None:
                actor_cfg.grit = self.config.grit
