...
CURVATURE_LAST_LINEAR_LAYERS="${CURVATURE_LAST_LINEAR_LAYERS:-${HVP_LAST_LINEAR_LAYERS:-0}}"
CURVATURE_BACKEND="${CURVATURE_BACKEND:-sam_fd}"
if [[ "${CURVATURE_BACKEND}" != "sam_fd" ]]; then
  echo "CURVATURE_BACKEND must be sam_fd" >&2
  exit 1
fi
...
echo "CURVATURE_BACKEND=${CURVATURE_BACKEND} SAM_RHO=${SAM_RHO} SAM_NORMALIZE_DIRECTION=${SAM_NORMALIZE_DIRECTION} CURVATURE_LAST_LINEAR_LAYERS=${CURVATURE_LAST_LINEAR_LAYERS}"
...
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" scripts/train_grit_dpo.py \
  ...
  --sam-rho "${SAM_RHO}" \
  "${SAM_ARGS[@]}" \
  --curvature-last-linear-layers "${CURVATURE_LAST_LINEAR_LAYERS}" \
  --dtype float16 \
  --trust-remote-code \
